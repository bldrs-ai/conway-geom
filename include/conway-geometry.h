...
struct GeometryStatistics {
  ...
  double GetCacheRatio() {
    return static_cast<double>(meshCacheHits) /
           (meshCacheHits + meshCacheMisses);
  }
};
...
class ConwayGeometryProcessor {
 public:
  ...
  IfcGeometry BoolSubtract(const IfcGeometry &firstGeom,
    ...
    if (USE_FAST_BOOLS /*LoaderSettings::USE_FAST_BOOLS*/) {
      ...
      const int threshold =
          BOOL_ABORT_THRESHOLD /*LoaderSettings().BOOL_ABORT_THRESHOLD*/;
  ...
  IfcGeometry GetBooleanResult(ParamsGetBooleanResult parameters) {
    // glm::dvec3 origin = GetOrigin(parameters.firstMesh,
    // parameters.firstMeshTranslation); auto normalizeMat =
    ...
    IfcGeometry resultGeometry =
        BoolSubtract(parameters.firstMesh, flatSecondGeoms);

    return resultGeometry;
  }

  IfcGeometry Extrude(IfcProfile profile, glm::dvec3 dir, double distance,
    ...
    if (parameters.isPlane) {
      ...
      if (parameters.UKnots[parameters.UKnots.size() - 1] !=
        ...
        for (uint32_t i = 0; i < parameters.UKnots.size(); i++) {
          parameters.UKnots[i] =
              parameters.UKnots[i] * (parameters.UKnots.size() - 1) /
      ...
      if (parameters.VKnots[parameters.VKnots.size() - 1] !=
        ...
        for (uint32_t i = 0; i < parameters.VKnots.size(); i++) {
          parameters.VKnots[i] =
              parameters.VKnots[i] * (parameters.VKnots.size() - 1) /
  ...
  void ComputePolylineCurve(IfcCurve<DIM> &curve,
  ...
  template <uint32_t DIM>
  void ComputeCompositeCurve(IfcCurve<DIM> &curve,
                             glm::vec<DIM, glm::f64> points, bool edge,
                             int sameSense = -1) {
    // todo: recursion
  }
  ...
  template <uint32_t DIM>
  void ComputeCompositeCurveSegment(IfcCurve<DIM> &curve,
                                    glm::vec<DIM, glm::f64> points, bool edge,
                                    int sameSense = -1) {
    // todo: recursion
  }

  IfcCurve<2> BuildArc3Pt(const glm::dvec2 &p1, const glm::dvec2 &p2,
                          const glm::dvec2 &p3) {
    ...
    while (pointList.size() <
           GeometryProcessorSettings::CIRCLE_SEGMENTS_MEDIUM) {
  ...
  // case ifc::IFCLINE
  template <uint32_t DIM>
  void ComputeCurveLine(IfcCurve<DIM> &curve, bool edge, TrimmingArguments trim,
                        int sameSense = -1) {
  ...
  template <uint32_t DIM>
  void ComputeTrimmedCurve(IfcCurve<DIM> &curve, glm::vec<DIM, glm::f64> points,
  ...
  template <uint32_t DIM>
  void ComputeCircleCurve(IfcCurve<DIM> &curve, glm::mat4 placementMat4,
  ...
  // split all 2D and 3D functions and get rid of the DIM template.
  template <uint32_t DIM>
  void ComputeEllipseCurve(IfcCurve<DIM> &curve, glm::mat4 placementMat4,
  ...
  */
  ...
  bool notPresent(glm::dvec3 pt, std::vector<glm::dvec3> points) {
    for (auto &pt2 : points)
      if (pt.x == pt2.x && pt.y == pt2.y && pt.z == pt2.z) return false;
    return true;
  }
  ...
  void TriangulateRevolution(IfcGeometry &geometry,
                             std::vector<IfcBound3D> &bounds,
                             conway::IfcSurface &surface) {
    ...
    // Now we find the angle of each point in the reference plane of the
    // cylinder
    ...
    // Add angular differences starting from the first angle. We also correct
    ...
    for (int i = 0; i < surface.RevolutionSurface.Profile.curve.points.size();
         i++) {
    ...
    for (int r = 0; r < numRots - 1; r++) {
      ...
      for (int s = 0; s < newPoints[r].size() - 1; s++) {
        geometry.AddFace(newPoints[r][s], newPoints[r][s + 1],
                         newPoints[r1][s]);
  ...
  void TriangulateCylindricalSurface(IfcGeometry &geometry,
                                     std::vector<IfcBound3D> &bounds,
                                     conway::IfcSurface &surface) {
    ...
    // Find the relative coordinates of each curve point in the cylinder
    // reference plane Only retain the max and min relative Z
    for (int i = 0; i < bounds.size(); i++) {
      for (int j = 0; j < bounds[i].curve.points.size(); j++) {
  ...
  void TriangulateExtrusion(IfcGeometry &geometry,
                            std::vector<IfcBound3D> &bounds,
                            conway::IfcSurface &surface) {
    ...
    if (!surface.ExtrusionSurface.Profile.isComposite) {
      ...
      for (uint32_t i = 0; i < surface.ExtrusionSurface.Profile.profiles.size();
           i++) {
        for (int j = 0;
             j <
             surface.ExtrusionSurface.Profile.profiles[i].curve.points.size() -
                 1;
             j++) {
          int j2 = j + 1;
          ...
          geometry.AddFace(
              glm::dvec3(
                  surface.ExtrusionSurface.Profile.profiles[i].curve.points[j],
                  0),
              glm::dvec3(
                  surface.ExtrusionSurface.Profile.profiles[i].curve.points[j2],
                  0),
              nptj1);
          geometry.AddFace(
              glm::dvec3(
  ...
  // TODO: review and simplify
  void TriangulateBspline(IfcGeometry &geometry,
                          std::vector<IfcBound3D> &bounds,
                          conway::IfcSurface &surface) {
    // double limit = 1e-4;
    ...
    if (tinynurbs::surfaceIsValid(srf)) {
      ...
      // r indicates the level of subdivision, currently 3 you can increase it
      // to 5
  ...
  void TriangulateBounds(IfcGeometry &geometry,
                         std::vector<IfcBound3D> &bounds) {
    if (bounds.size() == 1 && bounds[0].curve.points.size() == 3) {
      ...
      // if the outer bound is clockwise under the current projection
      // (v12,v13,n), we invert the projection
      if (!test.IsCCW()) {
        n *= -1;
        std::swap(v12, v13);
      }
  ...
  ResultsGltf GeometryToGltf(conway::IfcGeometry geom, bool isGlb,
    ...
    try {
      ...
      document.SetDefaultScene(
          std::move(scene), Microsoft::glTF::AppendIdPolicy::GenerateOnEmpty);
