...
#include "./is-inside-mesh-conway.h"
...
namespace conway
{
    ...
    manifold::Manifold * geom2manifold(const IfcGeometry& geometry) {
        manifold::Mesh mesh;
        ...
        if (manifold->Status() != manifold::Manifold::Error::NO_ERROR) {
              ...
              std:: cout << "Input Geometry to BooleanFunctions Not Manifold:"+errorName<<std::endl;
    ...
    IfcGeometry boolMultiOp_Manifold(const IfcGeometry& firstGeom, const std::vector<IfcGeometry>& secondGeoms)
    {
        ...
        manifold::Manifold * first = geom2manifold(firstGeom);
        ...
        {
            manifold::Manifold * holeManifold = geom2manifold(holeGeom);
