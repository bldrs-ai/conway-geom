...
struct StringView {
  StringView(char *d, uint32_t l) : data(d), len(l) {}
...
glm::dvec3 computeNormal(const glm::dvec3 v1, const glm::dvec3 v2,
                         const glm::dvec3 v3) {
  glm::dvec3 v12(v2 - v1);
  glm::dvec3 v13(v3 - v1);
...
bool IsInsideCenterExtents(const glm::dvec3 &pt, const glm::dvec3 &center,
...
struct IfcGeometry {
  ...
  glm::dvec3 GetExtent() const { return max - min; }
  ...
  uint32_t GetVertexDataSize() { return (uint32_t)fvertexData.size(); }

  uint32_t GetIndexData() { return (uint32_t)(size_t)&indexData[0]; }

  uint32_t GetIndexDataSize() { return (uint32_t)indexData.size(); }

  bool IsEmpty() { return vertexData.empty(); }
};
  ...
  inline void Add(const glm::vec<DIM, glm::f64> &pt) {
    if (points.empty()) {
      ...
      if (add) {
        ...
        // TODO: we are now discarding these points, but they probably should
        // not even be generated
      }
    }
  }

  void Invert() { std::reverse(points.begin(), points.end()); }
...
IfcCurve<2> GetEllipseCurve(float radiusX, float radiusY, int numSegments,
                            glm::dmat3 placement = glm::dmat3(1),
                            double startRad = 0, double endRad = CONST_PI * 2,
                            bool swap = true) {
  IfcCurve<2> c;
...
IfcCurve<2> GetCircleCurve(float radius, int numSegments,
                           glm::dmat3 placement = glm::dmat3(1)) {
  return GetEllipseCurve(radius, radius, numSegments, placement);
}

bool GetWindingOfTriangle(const glm::dvec3 &a, const glm::dvec3 &b,
                          const glm::dvec3 &c) {
  auto norm = computeNormal(a, b, c);
  return glm::dot(norm, glm::dvec3(0, 0, 1)) > 0.0;
}

bool TriangleIsCW(const glm::dvec2 &a, const glm::dvec2 &b,
                  const glm::dvec2 &c) {
  auto norm =
      computeNormal(glm::dvec3(a, 0), glm::dvec3(b, 0), glm::dvec3(c, 0));
  return glm::dot(norm, glm::dvec3(0, 0, 1)) > 0.0;
}
...
IfcCurve<2> GetRectangleCurve(double xdim, double ydim,
                              glm::dmat3 placement = glm::dmat3(1)) {
  double halfX = xdim / 2;
  double halfY = ydim / 2;
...
IfcCurve<2> GetIShapedCurve(double width, double depth, double webThickness,
...
IfcCurve<2> GetUShapedCurve(double depth, double flangeWidth,
...
IfcCurve<2> GetLShapedCurve(double width, double depth, double thickness,
...
glm::dvec2 BSplineInverseEvaluation(glm::dvec3 pt,
...
glm::dvec2 InterpolateRationalBSplineCurveWithKnots(
...
std::vector<glm::dvec2> GetRationalBSplineCurveWithKnots(
...
glm::dvec3 InterpolateRationalBSplineCurveWithKnots(
...
std::vector<glm::dvec3> GetRationalBSplineCurveWithKnots(
...
glm::dvec3 projectOntoPlane(const glm::dvec3 &origin, const glm::dvec3 &normal,
                            const glm::dvec3 &point, const glm::dvec3 &dir) {
...
bool GetBasisFromCoplanarPoints(std::vector<glm::dvec3> &points, glm::dvec3 &v1,
                                glm::dvec3 &v2, glm::dvec3 &v3) {
  v1 = points[0];
...
std::optional<glm::dvec3> GetOriginRec(
  ...
  if (geomIt != geometryMap.end()) {
    ...
    if (meshGeom.numFaces) {
      for (uint32_t i = 0; i < meshGeom.numFaces; i++) {
        ...
        printf(
            "GetOriginRec (Mesh(expressID: %i)): a - X: %.3f, Y: %.3f, Z: "
            "%.3f\n",
  ...
  for (auto &c : mesh.children) {
    ...
    if (v.has_value()) {
      printf(
          "GetOriginRec (Mesh(expressID: %i)): v - X: %.3f, Y: %.3f, Z: %.3f\n",
...
glm::dvec3 GetOrigin(IfcComposedMesh &mesh,
...
void flattenRecursive(IfcComposedMesh &mesh,
                      std::unordered_map<uint32_t, IfcGeometry> &geometryMap,
                      IfcGeometry &geom, glm::dmat4 mat) {
  glm::dmat4 newMat = mat * mesh.transformation;
...
std::vector<glm::dvec2> rescale(std::vector<glm::dvec2> input, glm::dvec2 size,
                                glm::dvec2 offset) {
  ...
  for (auto &pt : input) {
    // here we invert Y, since the canvas +y is down, but makes more sense to
    // think about +y as up
    retval.emplace_back(
...
std::string makeSVGLines(std::vector<glm::dvec2> input,
  ...
  svg << "<svg width=\"" << size.x + offset.x * 2 << "\" height=\""
      << size.y + offset.y * 2 << "\" xmlns=\"http://www.w3.org/2000/svg\" >";
  ...
  for (int i = 0; i < indices.size(); i += 3) {
    ...
    svg << "<polygon points=\"" << a.x << "," << a.y << " " << b.x << "," << b.y
        << " " << c.x << "," << c.y
        << "\" style=\"fill:gray; stroke:none; stroke - width:0\" />`;";
  }
...
void DumpSVGCurve(std::vector<glm::dvec3> points, glm::vec3 dir,
                  std::wstring filename, std::vector<uint32_t> indices = {}) {
...
struct Point {
  ...
  glm::dvec2 operator()() const { return glm::dvec2(x, y); }
};
...
glm::dvec2 cmin(glm::dvec2 m, Point p) {
  return glm::dvec2(std::min(m.x, p.x), std::min(m.y, p.y));
}

glm::dvec2 cmax(glm::dvec2 m, Point p) {
  return glm::dvec2(std::max(m.x, p.x), std::max(m.y, p.y));
}

Bounds getBounds(std::vector<Triangle> input, glm::dvec2 size,
                 glm::dvec2 offset) {
  std::vector<glm::dvec2> retval;
...
Bounds getBounds(std::vector<std::vector<glm::dvec2>> input, glm::dvec2 size,
                 glm::dvec2 offset) {
  std::vector<glm::dvec2> retval;
...
std::string makeSVGTriangles(std::vector<Triangle> triangles, Point p,
                             Point prev, std::vector<Point> pts = {}) {
  glm::dvec2 size(512, 512);
  glm::dvec2 offset(5, 5);
  ...
  if (p.id != -1) {
    svg << "<circle cx = \"" << rp.x << "\" cy = \"" << rp.y
        << "\" r = \"3\" style = \"stroke:rgb(0,0,255);stroke-width:2\" />";
  }

  if (prev.id != -1) {
    svg << "<circle cx = \"" << rprev.x << "\" cy = \"" << rprev.y
        << "\" r = \"3\" style = \"stroke:rgb(0,0,100);stroke-width:2\" />";
  }

  for (auto &pt : pts) {
    glm::dvec2 p = rescale(pt, bounds, size, offset);
    svg << "<circle cx = \"" << p.x << "\" cy = \"" << p.y
        << "\" r = \"1\" style = \"stroke:rgb(0,0,100);stroke-width:2\" />";
  }
...
void DumpSVGTriangles(std::vector<Triangle> triangles, Point p, Point prev,
                      std::wstring filename, std::vector<Point> pts = {}) {
  writeFile(filename, makeSVGTriangles(triangles, p, prev, pts));
}

void DumpSVGLines(std::vector<std::vector<glm::dvec2>> lines,
...
glm::dmat4 NormalizeIFC(glm::dvec4(1, 0, 0, 0), glm::dvec4(0, 0, -1, 0),
...
glm::dvec3 ToBary(const glm::dvec3 &a, const glm::dvec3 &b, const glm::dvec3 &c,
                  const glm::dvec3 &pt) {
  glm::dvec3 E1 = b - a;
  glm::dvec3 E2 = c - a;
  glm::dvec3 ROV0 = pt - a;
...
glm::dvec2 FromBary(const glm::dvec2 &a, const glm::dvec2 &b,
                    const glm::dvec2 &c, const glm::dvec3 &pt) {
...
glm::dvec3 FromBary(const glm::dvec3 &a, const glm::dvec3 &b,
                    const glm::dvec3 &c, const glm::dvec3 &pt) {
  return pt.x * a + pt.y * b + pt.z * c;
}
...
std::string ToObj(const IfcGeometry &geom, size_t &offset,
  ...
  for (uint32_t i = 0; i < geom.numFaces; i++) {
    ...
    obj << "f " << (f.i0 + 1 + offset) << "// " << (f.i1 + 1 + offset) << "// "
        << (f.i2 + 1 + offset) << "//\n";
  }
...
std::string ToObj(IfcComposedMesh &mesh,
                  std::unordered_map<uint32_t, IfcGeometry> &geometryMap,
                  size_t &offset, glm::dmat4 mat = glm::dmat4(1)) {
  std::string complete;
...
// This class defines a datachunk that can be loaded and unloaded from an
...
class chunkUnit {
 public:
  ...
  void initialize() { chunk.resize(N); }
...
class DynamicTape {
 public:
  ...
  inline void push2(uint16_t v) { push(&v, 2); }
  ...
  inline void SetWriteAtEnd() { writePtr = chunks.size() - 1; }

  uint64_t GetTotalSize() {
    return (chunks.size() - 1) * N + chunks.back().size;
  }

  uint64_t GetCapacity() { return chunks.size() * N; }
  ...
  inline bool AtEnd() { return readChunkIndex == chunks.size(); }

  inline uint32_t GetReadOffset() { return readChunkIndex * N + readPtr; }
  ...
  void DumpToDisk(uint32_t num) {
    ...
    std::string str;
    ss >> str;
    std::ofstream file(serializedFileName + "_" + str + ".bin",
  ...
  uint32_t Copy(uint32_t offset, uint32_t endOffset, uint8_t *dest) {
    ...
    if (chunkStart == chunkEnd) {
      std::memcpy(dest, &chunks[chunkStart].chunk[chunkStartPos],
      ...
      std::memcpy(dest, &chunks[chunkStart].chunk[chunkStartPos],
                  partOfStartchunk);
      std::memcpy(dest + partOfStartchunk, &chunks[chunkEnd].chunk[0],
                  chunkEndPos);
      updateChunks();
