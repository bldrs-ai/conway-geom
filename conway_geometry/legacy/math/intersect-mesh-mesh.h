...
namespace conway
{
    ...
    static std::vector<conway::geometry::Loop> makeLoops(const glm::dvec3& a, const glm::dvec3& b, const glm::dvec3& c, const std::vector<MeshIntersection>& intersections)
    {
        std::vector<conway::geometry::Loop> loops;
        ...
        for (auto& i : intersections)
        {
            ...
            // single point
            if (fuzzybools::equals2d(ps, pe, EPS_SMALL))
            {
    ...
    static std::vector<conway::geometry::Loop> makeLoops2(const glm::dvec3& A, const glm::dvec3& B, const glm::dvec3& C, const glm::dvec2& a, const glm::dvec2& b, const glm::dvec2& c, const std::vector<MeshIntersection>& intersections)
    {
        std::vector<conway::geometry::Loop> loops;

        for (auto& i : intersections)
        {
            glm::dvec3 psb = fuzzybools::ToBary(A, B, C, i.result.start);
            glm::dvec3 peb = fuzzybools::ToBary(A, B, C, i.result.end);
            glm::dvec2 ps = fuzzybools::FromBary(a, b, c, psb);
            glm::dvec2 pe = fuzzybools::FromBary(a, b, c, peb);
        ...
        for (uint32_t i = 0; i < mesh.numFaces; i++)
        {
            ...
            if (!ints.empty())
            {
                ...
                if (!(loops.size() == 1 && loops[0].hasOne))
                {
                    ...
                    std::transform(loops.begin(), loops.end(), std::back_inserter(vecs), [](const conway::geometry::Loop& l) -> std::vector<glm::dvec2>
                                   {
                                       return { l.v1, l.v2 };
                                   });
                ...
                std::vector<conway::TriangulateWithBoundaries::Triangle> tesselation = twb.triangulate(pa, pb, pc, loops, swapped);
                ...
                for (auto& triangle : tesselation)
                {
                    if (triangle.id != -1)
                    {
                        ...
                        glm::dvec3 upa = fuzzybools::FromBary(a, b, c, fuzzybools::ToBary2(ta));// unProjectFromTriangle(ta, a, b, c);
                        glm::dvec3 upb = fuzzybools::FromBary(a, b, c, fuzzybools::ToBary2(tb));//unProjectFromTriangle(tb, a, b, c);
                        glm::dvec3 upc = fuzzybools::FromBary(a, b, c, fuzzybools::ToBary2(tc));//unProjectFromTriangle(tc, a, b, c);
    ...
    static int MakeBVH(std::vector<fuzzybools::AABB>& boxes, std::vector<BVHNode>& nodes, int start, int end, int axis, int depth, int& offset)
    {
        int nodeID = offset++;
        ...
        std::nth_element(boxes.begin() + start, boxes.begin() + middle, boxes.begin() + end, [&](const fuzzybools::AABB& first, const fuzzybools::AABB& second)
                         {
    ...
    static BVH MakeBVH(const geometry::IfcGeometry& mesh)
    {
        BVH bvh;
        bvh.ptr = &mesh;
        bvh.boxes = std::vector<fuzzybools::AABB>(mesh.numFaces);
        for (uint32_t i = 0; i < mesh.numFaces; i++)
        {
                                    ...
                                    const std::unordered_map<size_t, size_t>& edgeMap)
    {
        ...
        while (currentVertex != initial.first)
        {
            ...
            {
                printf("conway::geometry::Looping while finding contour!\n");
    ...
    struct Polygon3D
    {
        ...
        std::vector<conway::geometry::Face> faces;
        ...
        glm::dvec2 Project2D(const glm::dvec3& pt) const
        {
            ...
            glm::dvec3 bpt = fuzzybools::ToBary(a, b, c, pt);
            return fuzzybools::FromBary(pa, pb, pc, bpt);
        }
        ...
        glm::dvec3 Unproject2D(const glm::dvec2& pt) const
        {
            ...
            glm::dvec3 bpt = fuzzybools::ToBary2(pt);
            return fuzzybools::FromBary(a, b, c, bpt);
        }
        ...
        void TriangulateCDT(const std::vector<std::vector<std::vector<size_t>>>& parentLoops, geometry::IfcGeometry& outputGeom, bool eraseHoles = false)
        {
            for (auto& parentLoop : parentLoops)
            {
                ...
                for (auto& t : triangles)
                {
                    ...
                    double area = geometry::areaOfTriangle(a2d, b2d, c2d);
                    if (area < EPS_SMALL)
                    {
                        printf("small area");
                    }
...
//                Dumpgeometry::IfcGeometry(outputGeom, L"output.obj");
            }
        }
        ...
        size_t AddPoint(const glm::dvec2& pt, const glm::dvec3& pt3, bool checkIntersect = false)
        {
            ...
            for (size_t i = 0; i < points.size(); i++)
            {
                if (fuzzybools::equals2d(pt, points[i], EPS_BIG))
                {
                    if (index != -1)
                    {
    ...
    {
        Plane p;
        p.norm = geometry::computeNormal(a, b, c);
    ...
    static BrepMesh MeshToBReps(const geometry::IfcGeometry& mesh, size_t meshIndex)
    {
        ...
        {
            const geometry::Face& f = mesh.GetFace(fi);
    ...
    void intersectMeshMesh(const geometry::IfcGeometry& mesh1, const geometry::IfcGeometry& mesh2, geometry::IfcGeometry& result1, geometry::IfcGeometry& result2)
    {
        ...
        for (uint32_t i = 0; i < mesh1.numFaces; i++)
        {
            ...
            {
                geometry::Face t1 = mesh1.GetFace(i);
                geometry::Face t2 = mesh2.GetFace(j);
