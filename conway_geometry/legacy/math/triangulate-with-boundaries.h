...
namespace conway
{
	...
	struct Triangle
	{
	    ...
	    {
		    auto norm = geometry::computeNormal(glm::dvec3(a, 0), glm::dvec3(b, 0), glm::dvec3(c, 0));
        ...
        bool makeTriangle(std::vector<Triangle>& triangles, const Point& a, const Point& b, const Point& c)
        {
            ...
            double area = geometry::areaOfTriangle(t.a(), t.b(), t.c());
        ...
        std::vector<Triangle> triangulate(const glm::dvec2& a, const glm::dvec2& b, const glm::dvec2& c, std::vector<conway::geometry::Loop>& loops, bool& swapped)
        {
            instance++;
