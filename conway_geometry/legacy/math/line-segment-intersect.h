...
namespace conway
{
	...
	bool doLineSegmentsIntersect(const glm::dvec2& p, const glm::dvec2& p2, const glm::dvec2& q, const glm::dvec2& q2, bool extendQ = false) {
		...
		double uNumerator = geometry::cross2d(q - p, r);
		double denominator = geometry::cross2d(r, s);

		if (uNumerator == 0 && denominator == 0) {
			// They are coLlinear

			// Do they touch? (Are any of the points equal?)
			if (fuzzybools::equals2d(p, q) || fuzzybools::equals2d(p, q2) || fuzzybools::equals2d(p2, q) || fuzzybools::equals2d(p2, q2))
			{
				return true;
			}
		...
		double t = geometry::cross2d(q - p, s) / denominator;
