...
namespace conway
{
	...
	struct SVGLineSet
	{
		...
		fuzzybools::Bounds GetBounds(glm::dvec2 size, glm::dvec2 offset)
		{
			return fuzzybools::getBounds(lines, size, offset);
			;
		}
	};
	...
	static void SVGLinesToString(fuzzybools::Bounds bounds, glm::dvec2 size, glm::dvec2 offset, SVGLineSet lineSet, std::stringstream &svg)
	{
	...
	static std::string makeSVGLines(SVGDrawing drawing)
	{
		...
		fuzzybools::Bounds bounds = drawing.GetBounds(size, offset);
	...
	static std::string ToObj(const geometry::Geometry &geom, size_t &offset, glm::dmat4 transform = glm::dmat4(1), double inputScale = 1.0)
	{
		...
		{
			geometry::Face f = geom.GetFace(i);
