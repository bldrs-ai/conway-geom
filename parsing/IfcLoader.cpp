...
namespace webifc::parsing {
  ...
  std::string p21decode(std::string_view & str);    
   ...
   { 
      if (expressID == 0 || expressID > _lines.size()) {
        printf("[GetLineType()] Attempt to Access Invalid ExpressID {}", expressID);
        return 0;
      }
  ...
  //gcc c++20 doesn't have STD format yet:
  void IfcLoader::PushDouble(double input)
  {             
  ...
  uint32_t IfcLoader::GetCurrentLineExpressID() const
  {
   ...
   { 
      if (_tokenStream->Read<char>() != IfcTokenType::REF)
     	{
     		printf("[GetRefArgument()] unexpected token type, expected REF {}", GetCurrentLineExpressID());
     		return 0;
     	}
  ...
  IfcTokenType IfcLoader::GetTokenType(uint32_t tapeOffset) const
  {
   ...
   { 
      IfcTokenType t = GetTokenType();
     	...
     	{
     		printf("[GetOptionalRefArgument()] unexpected token type, expected REF or EMPTY", GetCurrentLineExpressID());
     		return 0;
     	}
   }
   ...
   { 
     ...
     while (true)
     {
       ...
       else
       {
         ...
         {
           printf("[GetSetArgument[]) unexpected token", GetCurrentLineExpressID());
         }
       }
   	 ...
   	 std::vector<uint32_t> tempSet;
     	...
     	while (true)
     	{
     		...
     		else
     		{
     			...
     			{
     				printf("[GetSetListArgument()] unexpected token", GetCurrentLineExpressID());
     			}
     		}
   ...
   void IfcLoader::ArgumentOffset(const uint32_t argumentIndex) const
   {
   	...
   	while (true)
   	{
   		...
   		case IfcTokenType::LINE_END:
   		{
   			printf("[ArgumentOffset()] unexpected line end {}", GetCurrentLineExpressID());
   			break;
   		}
   		case IfcTokenType::UNKNOWN:
   		case IfcTokenType::EMPTY:
