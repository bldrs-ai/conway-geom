...
class IfcLoader {
 public:
  IfcLoader(size_t tapeSize, size_t memoryLimit,
  ...
  const std::vector<IfcHeaderLine> GetHeaderLinesWithType(
  ...
  bool IsAtEnd() const;
  void SetClosed();
  void MoveToLineArgument(const uint32_t lineID,
                          const uint32_t argumentIndex) const;
  void MoveToHeaderLineArgument(const uint32_t lineID,
                                const uint32_t argumentIndex) const;
  ...
  void MoveToArgumentOffset(const IfcLine &line,
                            const uint32_t argumentIndex) const;
  void StepBack() const;
  IFC_SCHEMA GetSchema() const;
  void Push(void *v, const uint64_t size);
  uint64_t GetTotalSize() const;
  void UpdateLineTape(const uint32_t expressID, const uint32_t type,
