...
IfcTokenStream::IfcTokenChunk::IfcTokenChunk(const size_t chunkSize,
                                             const size_t startRef,
                                             const size_t fileStartRef,
                                             IfcFileStream* fileStream)
    : _startRef(startRef),
...
size_t IfcTokenStream::IfcTokenChunk::GetTokenRef() { return _startRef; }

size_t IfcTokenStream::IfcTokenChunk::TokenSize() { return _currentSize; }

bool IfcTokenStream::IfcTokenChunk::IsLoaded() { return _loaded; }

std::string_view IfcTokenStream::IfcTokenChunk::ReadString(const size_t ptr,
                                                           const size_t size) {
  if (!_loaded) Load();
...
void IfcTokenStream::IfcTokenChunk::Load() {
  ...
  while (!_fileStream->IsAtEnd() && _currentSize < _chunkSize) {
    ...
    if (c == '\'') {
      ...
      // this example from uptown shows that escaping is not used: 'Type G5 -
      // 800kg/m\X2\00B2\X0\'; this example from revit shows that double quotes
      // are used as one quote: 'RPC Tree - Deciduous:Scarlet Oak - 42'':946835'
      ...
      if (_fileStream->Prev() == '/') {
        ...
        Push<uint8_t>(IfcTokenType::UNKNOWN);
    } else if (c == '(')
      Push<uint8_t>(IfcTokenType::SET_BEGIN);
    else if (c >= '0' && c <= '9') {
      bool negative = _fileStream->Prev() == '-';
      temp.clear();

      while ((_fileStream->Get() >= '0' && _fileStream->Get() <= '9') ||
             (_fileStream->Get() == '.') || _fileStream->Get() == 'e' ||
             _fileStream->Get() == 'E' || _fileStream->Get() == '-' ||
             _fileStream->Get() == '+') {
        temp.push_back(_fileStream->Get());
        _fileStream->Forward();
      }
      ...
      temp.clear();
      while ((_fileStream->Get() >= 'A' && _fileStream->Get() <= 'Z') ||
             (_fileStream->Get() >= 'a' && _fileStream->Get() <= 'z') ||
             (_fileStream->Get() >= '0' && _fileStream->Get() <= '9') ||
             _fileStream->Get() == '_') {
      ...
      Push<uint8_t>(IfcTokenType::SET_END);
    else if (c == ';') {
