...
void IfcTokenStream::SetTokenSource(
  ...
  while (!_fileStream->IsAtEnd()) {
    checkMemory();
    IfcTokenChunk chunk(_chunkSize, tokenOffset, _fileStream->GetRef(),
                        _fileStream);
...
void IfcTokenStream::Push(void *v, const size_t size) {
  ...
  if (_chunks.back().TokenSize() + size > _chunkSize) {
    ...
    _chunks.emplace_back(
        _chunkSize, _chunks.back().GetTokenRef() + _chunks.back().TokenSize(),
        0, _fileStream);
    _activeChunks++;
  }
...
bool IfcTokenStream::IsAtEnd() {
  return _currentChunk >= _chunks.size() - 1 &&
         _readPtr >= _chunks.back().TokenSize();
}
