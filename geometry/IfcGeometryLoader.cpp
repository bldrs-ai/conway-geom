...
IfcCrossSections IfcGeometryLoader::GetCrossSections2D(
    uint32_t expressID) const {
...
IfcCrossSections IfcGeometryLoader::GetCrossSections3D(
    uint32_t expressID, bool scaled, glm::dmat4 coordination) const {
  auto lineType = _loader.GetLineType(expressID);
  IfcCrossSections sections;
  double scale = 1;
  if (scaled) {
    ...
    case schema::IFCSECTIONEDSOLIDHORIZONTAL: {
      ...
      for (auto &face : faces) {
        ...
        for (uint32_t i = 0; i < profile.curve.points.size(); i++) {
          glm::dvec3 pTemp =
              transform[id] * glm::dvec4(profile.curve.points[i], 1);
          profile.curve.points[i] =
              coordination * glm::dvec4(pTemp.x, pTemp.y, pTemp.z, 1);
        }
        profiles.push_back(profile);
        curves.push_back(profile.curve);
        expressIds.push_back(expressID);
...
IfcAlignment IfcGeometryLoader::GetAlignment(uint32_t expressID,
    ...
    case schema::IFCALIGNMENT: {
      ...
      if (relAggVector.count(expressID) == 1) {
        ...
        for (auto expressID : relAgg) {
          alignment =
              GetAlignment(expressID, alignment, transform * transform_t);
        }
      }
    ...
    case schema::IFCALIGNMENTHORIZONTAL: {
      ...
      if (relAggVector.count(expressID) == 1) {
        ...
        for (size_t i = 0; i < alignment.Horizontal.curves.size(); i++) {
          for (size_t j = 0; j < alignment.Horizontal.curves[i].points.size();
               j++) {
            alignment.Horizontal.curves[i].points[j] =
                glm::dvec4(alignment.Horizontal.curves[i].points[j].x,
                           alignment.Horizontal.curves[i].points[j].y, 0, 1) *
    ...
    case schema::IFCALIGNMENTVERTICAL: {
      ...
      if (relAggVector.count(expressID) == 1) {
        ...
        for (size_t i = 0; i < alignment.Vertical.curves.size(); i++) {
          for (size_t j = 0; j < alignment.Vertical.curves[i].points.size();
               j++) {
            alignment.Vertical.curves[i].points[j] =
                glm::dvec4(alignment.Vertical.curves[i].points[j].x,
                           alignment.Vertical.curves[i].points[j].y, 0, 1) *
                transform * transform_t;
          }
        }
      }
  ...
  switch (lineType) {
    case schema::IFCALIGNMENTSEGMENT: {
      ...
      for (size_t i = 0; i < alignmentCurve.points.size(); i++) {
        alignmentCurve.points[i] =
            glm::dvec4(alignmentCurve.points[i].x, alignmentCurve.points[i].y,
                       0, 1) *
            transform_t;
      }
    ...
    case schema::IFCALIGNMENTHORIZONTALSEGMENT: {
      ...
      switch (Horizontal_alignment_type.at(type)) {
        ...
        {
          IfcCurve curve;
          glm::dvec2 Direction(glm::cos(ifcStartDirection),
        ...
        case 2:  // ARC
        {
          ...
          bool sw = true;
          auto curve2D = GetEllipseCurve(
              StartRadiusOfCurvature, StartRadiusOfCurvature, 20, glm::dmat3(1),
              ifcStartDirection, ifcStartDirection + span, sw);
          glm::dvec2 desp = glm::dvec2(StartPoint.x - curve2D.points[0].x,
                                       StartPoint.y - curve2D.points[0].y);
        ...
        case 3:  // CLOTHOID
        {
          ...
          IfcCurve curve;
          double A = sqrt(abs(EndRadiusOfCurvature - StartRadiusOfCurvature) *
                          SegmentLength);
          ...
          glm::dvec2 DirectionX(glm::cos(ifcStartDirection),
                                glm::sin(ifcStartDirection));
          glm::dvec2 DirectionY(-glm::sin(ifcStartDirection),
          ...
          if (inverse) {
            ...
            glm::dvec2 newDirectionX(endDir.x, endDir.y);
            glm::dvec2 newDirectionY(-endDir.y, endDir.x);
            ...
            for (uint32_t i = 0; i < curve.points.size(); i++) {
              ...
              double newDx =
                  StartPoint.x + DirectionX.x * dx + DirectionY.x * dy;
              double newDy =
                  StartPoint.y + DirectionX.y * dx + DirectionY.y * dy;
    ...
    case schema::IFCALIGNMENTVERTICALSEGMENT: {
      ...
      switch (Vertical_alignment_type.at(type)) {
        ...
        case 1:  // CONSTANTGRADIENT
        {
          ...
          glm::dvec3 jPoint =
              glm::dvec3(StartDistAlong + HorizontalLength,
                         StartHeight + HorizontalLength * StartGradient, 1);
        ...
        case 2:  // ARC
        {
          ...
          auto curve2D = GetEllipseCurve(
              RadiusOfCurvature, RadiusOfCurvature, _circleSegments,
              glm::dmat3(1), ifcStartDirection, ifcEndDirection, sw);
          glm::dvec2 desp = glm::dvec2(StartPoint.x - curve2D.points[0].x,
                                       StartPoint.y - curve2D.points[0].y);
        ...
        case 3:  // PARABOLIC
        {
          ...
          for (double i = 0; i <= _circleSegments; i++) {
            ...
            double alt =
                (HorizontalLength * pr * (grad + StartGradient) * 0.5) +
                StartHeight;
            points.push_back(glm::dvec2(HorizontalLength * pr, alt));
          }

          glm::dvec2 desp = glm::dvec2(StartPoint.x - points[0].x,
...
std::optional<glm::dvec4> IfcGeometryLoader::GetColor(
    uint32_t expressID) const {
  auto lineType = _loader.GetLineType(expressID);
  webifc::statistics::collectStatistics(lineType);
  webifc::statistics::printTypeInfo(
  ...
  switch (lineType) {
    ...
    case schema::IFCSURFACESTYLE: {
      ...
      for (auto &styleElementSelect : ifcSurfaceStyleElementSelects) {
        uint32_t styleElementSelectID =
            _loader.GetRefArgument(styleElementSelect);
    ...
    case schema::IFCMATERIALCONSTITUENTSET: {
      ...
      for (auto &materialContituent : materialContituents) {
        uint32_t materialContituentID =
            _loader.GetRefArgument(materialContituent);
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected style type", expressID, lineType);
      break;
  }
...
IfcBound3D IfcGeometryLoader::GetBound(uint32_t expressID) const {
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected bound type", expressID, lineType);
      break;
  }
  ...
  switch (lineType) {
    ...
    case schema::IFCEDGELOOP: {
      ...
      for (auto &token : edges) {
        ...
        // possible, otherwise we must check
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected loop type", expressID, lineType);
      break;
  }
...
glm::dvec3 IfcGeometryLoader::GetVertexPoint(uint32_t expressID) const {
  ...
  if (point == schema::IFCCARTESIANPOINT) {
    ...
    _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                              "unexpected vertxpoint type", pointRef, point);
    return {};
  }
}
...
IfcCurve IfcGeometryLoader::GetEdge(uint32_t expressID) const {
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected edgecurve type", expressID,
                                lineType);
      break;
  }
  return IfcCurve();
...
IfcTrimmingSelect IfcGeometryLoader::GetTrimSelect(
    uint32_t DIM, std::vector<uint32_t> &tapeOffsets) const {
...
glm::dvec3 IfcGeometryLoader::GetCartesianPoint3D(
    const uint32_t expressID) const {
  _loader.MoveToArgumentOffset(expressID, 0);
  _loader.GetTokenType();
  // because these calls cannot be reordered we have to use intermediate
  // variables
...
glm::dvec2 IfcGeometryLoader::GetCartesianPoint2D(
    const uint32_t expressID) const {
  _loader.MoveToArgumentOffset(expressID, 0);
  _loader.GetTokenType();
  // because these calls cannot be reordered we have to use intermediate
...
std::vector<glm::dvec3> IfcGeometryLoader::ReadIfcCartesianPointList3D(
    uint32_t expressID) const {
  _loader.MoveToArgumentOffset(expressID, 0);
...
std::vector<glm::dvec2> IfcGeometryLoader::ReadIfcCartesianPointList2D(
    uint32_t expressID) const {
  _loader.MoveToArgumentOffset(expressID, 0);
...
IfcCurve IfcGeometryLoader::GetCurve(uint32_t expressID, uint8_t dimensions,
                                     bool edge) const {
  IfcCurve curve;
...
void IfcGeometryLoader::ComputeCurve(uint32_t expressID, IfcCurve &curve,
                                     uint8_t dimensions, bool edge,
                                     int sameSense, int trimSense,
                                     IfcTrimmingArguments trim) const {
  auto lineType = _loader.GetLineType(expressID);
  webifc::statistics::collectStatistics(lineType);
  webifc::statistics::printTypeInfo(
      "//%s\n", (_schemaManager.IfcTypeCodeToType(lineType)).c_str());
  switch (lineType) {
    ...
    case schema::IFCCOMPOSITECURVE: {
      ...
      if (selfIntersects == "T") {
        ...
        _errorHandler.ReportError(utility::LoaderErrorType::UNSPECIFIED,
                                  "Self intersecting composite curve",
                                  expressID);
      }
    ...
    case schema::IFCLINE: {
      ...
      if (dimensions == 2 && trim.exist) {
        if (trim.start.hasPos && trim.end.hasPos) {
          ...
          _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                    "Unsupported trimmingselect 2D IFCLINE",
                                    expressID, lineType);
        }
        ...
        if (trim.start.hasPos && trim.end.hasPos) {
          ...
          _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                    "Unsupported trimmingselect 3D IFCLINE",
                                    expressID, lineType);
        }
      }
      break;
    ...
    case schema::IFCTRIMMEDCURVE: {
      ...
      ComputeCurve(basisCurveID, curve, dimensions, edge, sameSense,
                   senseAgreement, trim);
    ...
    case schema::IFCINDEXEDPOLYCURVE: {
      ...
      if (_loader.GetTokenType() != parsing::IfcTokenType::EMPTY) {
        ...
        if (selfIntersects == "T") {
          // TODO: this is probably bad news
          _errorHandler.ReportError(utility::LoaderErrorType::UNSPECIFIED,
                                    "Self intersecting ifcindexedpolycurve",
                                    expressID);
        }
      }
      ...
      if (dimensions == 2) {
        ...
        if (_loader.GetTokenType() != parsing::IfcTokenType::EMPTY) {
          ...
          for (auto &sg : pnSegment) {
            ...
            if (sg.type == "IFCARCINDEX") {
              auto pts = ReadIfcCartesianPointList2D(ptsRef);
              IfcCurve arc =
                  BuildArc3Pt(pts[sg.indexs[0] - 1], pts[sg.indexs[1] - 1],
                              pts[sg.indexs[2] - 1], _circleSegments);
        ...
        if (_loader.GetTokenType() != parsing::IfcTokenType::EMPTY) {
          ...
          for (auto &sg : pnSegment) {
            ...
            if (sg.type == "IFCARCINDEX") {
              auto pts = ReadIfcCartesianPointList3D(ptsRef);
              IfcCurve arc = Build3DArc3Pt(
                  pts[sg.indexs[0] - 1], pts[sg.indexs[1] - 1],
                  pts[sg.indexs[2] - 1], _circleSegments, EPS_MINISCULE);
              for (auto &pt : arc.points) {
                curve.Add(pt);
              }
            }
        ...
        _errorHandler.ReportError(
            utility::LoaderErrorType::UNSPECIFIED,
            "Parsing ifcindexedpolycurve in 3D is not possible", expressID);
      }
      ...
      for (int i = 0; i < _circleSegments; i++) {
        ...
        if (dimensions == 2) {
          ...
          vec[1] =
              radius *
              std::sin(
                  angle);  // not sure why we need this, but we apparently do
    ...
    case schema::IFCGRADIENTCURVE: {
      ...
      // #ifdef DEBUG_DUMP_SVG
      //   webifc::io::DumpGradientCurve(curveList, curve,"V_gradient.obj",
      //   "H_gradient.obj");
      // #endif
      break;
    }
    case schema::IFCCURVESEGMENT: {
      ...
      for (size_t j = 0; j < curve.points.size(); j++) {
        curve.points[j] =
            placement *
            glm::dvec3(curve.points[j].x, curve.points[j].y, curve.points[j].z);
      }
      break;
    }
    case schema::IFCBSPLINECURVE: {
      ...
      if (knots.size() != points.size() + degree + 1) {
        std::cout << "Error: Knots and control points do not match"
                  << std::endl;
      }
    ...
    case schema::IFCBSPLINECURVEWITHKNOTS: {
      ...
      // values amongst the d+k+2 knots in the knot list; L will be referred to
      // as the ‘upper index on knots’. Let mj denote the multiplicity (i.e.,
      // number of repetitions) of the _j_th distinct knot.
      ...
      auto closed = _loader.GetStringArgument();
      auto selfIntersect = _loader.GetStringArgument();
      auto knotMultiplicitiesSet =
          _loader.GetSetArgument();  // The multiplicities of the knots. This
                                     // list defines the number of times each
                                     // knot in the knots list is to be repeated
                                     // in constructing the knot array.
      auto knotSet =
          _loader.GetSetArgument();  // The list of distinct knots used to
    ...
    case schema::IFCRATIONALBSPLINECURVEWITHKNOTS: {
      ...
      auto knotSpec =
          _loader.GetStringArgument();  // The description of the knot type.
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "Unsupported curve type", expressID, lineType);
      break;
  }
  ...
  switch (lineType) {
    ...
    case schema::IFCRECTANGLEHOLLOWPROFILEDEF: {
      ...
      profile.holes.push_back(
          GetRectangleCurve(xdim - thickness, ydim - thickness, placement));
      std::reverse(profile.holes[0].points.begin(),
                   profile.holes[0].points.end());
    ...
    case schema::IFCELLIPSEPROFILEDEF: {
      ...
      profile.curve =
          GetEllipseCurve(radiusX, radiusY, _circleSegments, placement);
    ...
    case schema::IFCCIRCLEHOLLOWPROFILEDEF: {
      ...
      profile.curve = GetCircleCurve(radius, _circleSegments, placement);
      profile.holes.push_back(
          GetCircleCurve(radius - thickness, _circleSegments, placement));
      std::reverse(profile.holes[0].points.begin(),
                   profile.holes[0].points.end());
    ...
    case schema::IFCISHAPEPROFILEDEF: {
      ...
      profile.curve =
          GetIShapedCurve(width, depth, webThickness, flangeThickness,
                          hasFillet, filletRadius, placement);
    ...
    case schema::IFCLSHAPEPROFILEDEF: {
      ...
      profile.curve =
          GetLShapedCurve(width, depth, thickness, hasFillet, filletRadius,
                          edgeRadius, legSlope, placement);
    ...
    case schema::IFCTSHAPEPROFILEDEF: {
      ...
      profile.curve =
          GetTShapedCurve(width, depth, webThickness, hasFillet, filletRadius,
                          flangeEdgeRadius, flangeSlope, placement);
    ...
    case schema::IFCUSHAPEPROFILEDEF: {
      ...
      profile.curve =
          GetUShapedCurve(depth, flangeWidth, webThickness, flangeThickness,
                          filletRadius, edgeRadius, flangeSlope, placement);
    ...
    case schema::IFCCSHAPEPROFILEDEF: {
      ...
      profile.curve = GetCShapedCurve(Width, depth, girth, Thickness, hasFillet,
                                      filletRadius, placement);
    ...
    case schema::IFCZSHAPEPROFILEDEF: {
      ...
      profile.curve =
          GetZShapedCurve(depth, flangeWidth, webThickness, flangeThickness,
                          filletRadius, edgeRadius, placement);
    ...
    case schema::IFCDERIVEDPROFILEDEF: {
      ...
      if (!profile.isComposite) {
        for (uint32_t i = 0; i < profile.curve.points.size(); i++) {
          profile.curve.points[i] =
              transformation * glm::dvec3(profile.curve.points[i].x,
                                          profile.curve.points[i].y, 1);
        }
      } else {
        for (uint32_t j = 0; j < profile.profiles.size(); j++) {
          for (uint32_t i = 0; i < profile.profiles[j].curve.points.size();
               i++) {
            profile.profiles[j].curve.points[i] =
                transformation *
                glm::dvec3(profile.profiles[j].curve.points[i].x,
                           profile.profiles[j].curve.points[i].y, 1);
          }
        }
      }
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected profile type", expressID, lineType);
      break;
  }
...
IfcProfile IfcGeometryLoader::GetProfile3D(uint32_t expressID) const {
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected 3D profile type", expressID,
                                lineType);
      break;
  }
...
glm::dmat3 IfcGeometryLoader::GetAxis2Placement2D(uint32_t expressID) const {
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected 2D placement type", expressID,
                                lineType);
      break;
  }
  return glm::dmat3();
...
glm::dmat4 IfcGeometryLoader::GetLocalPlacement(uint32_t expressID,
  ...
  switch (lineType) {
    ...
    case schema::IFCAXIS2PLACEMENT3D: {
      ...
      if (zID == parsing::IfcTokenType::REF) {
        _loader.StepBack();
        auto tmpVec =
            glm::normalize(GetCartesianPoint3D(_loader.GetRefArgument()));
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected placement type", expressID,
                                lineType);
      break;
  }
...
std::array<glm::dvec3, 2> IfcGeometryLoader::GetAxis1Placement(
    const uint32_t expressID) const {
  _loader.MoveToArgumentOffset(expressID, 0);
  uint32_t locationID = _loader.GetRefArgument();
  parsing::IfcTokenType dirToken = _loader.GetTokenType();
...
IfcGeometryLoader::PopulateRelVoidsMap() {
...
std::unordered_map<uint32_t, std::vector<uint32_t>>
IfcGeometryLoader::PopulateRelVoidsRelMap() {
  std::unordered_map<uint32_t, std::vector<uint32_t>> resultVector;
  auto relVoids = _loader.GetExpressIDsWithType(schema::IFCRELVOIDSELEMENT);
...
std::unordered_map<uint32_t, std::vector<uint32_t>>
IfcGeometryLoader::PopulateRelAggregatesMap() {
  std::unordered_map<uint32_t, std::vector<uint32_t>> resultVector;
...
std::unordered_map<uint32_t, std::vector<uint32_t>>
IfcGeometryLoader::PopulateRelElementAggregatesMap() {
  std::unordered_map<uint32_t, std::vector<uint32_t>> resultVector;
...
IfcGeometryLoader::PopulateStyledItemMap() {
  std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>>
      returnVector;
  ...
  for (uint32_t styledItemID : styledItems) {
    ...
    if (_loader.GetTokenType() == parsing::IfcTokenType::REF) {
      ...
      for (auto &styleAssignment : styleAssignments) {
        ...
        returnVector[representationItem].emplace_back(styledItemID,
                                                      styleAssignmentID);
      }
    }
  }
...
std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>>
IfcGeometryLoader::PopulateRelMaterialsMap() {
  std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>>
      resultVector;
  auto styledItems =
      _loader.GetExpressIDsWithType(schema::IFCRELASSOCIATESMATERIAL);
...
std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>>
IfcGeometryLoader::PopulateMaterialDefinitionsMap() {
  std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>>
      resultVector;
  auto matDefs = _loader.GetExpressIDsWithType(
      schema::IFCMATERIALDEFINITIONREPRESENTATION);
...
void IfcGeometryLoader::ReadLinearScalingFactor() {
  ...
  if (projects.size() != 1) {
    _errorHandler.ReportError(utility::LoaderErrorType::PARSING,
...
IfcGeometryLoader::GetRelVoids() const {
  return _relVoids;
}

const std::unordered_map<uint32_t, std::vector<uint32_t>> &
IfcGeometryLoader::GetRelVoidRels() const {
  return _relVoidRel;
}

const std::unordered_map<uint32_t, std::vector<uint32_t>> &
IfcGeometryLoader::GetRelAggregates() const {
  return _relAggregates;
}

const std::unordered_map<uint32_t, std::vector<uint32_t>> &
IfcGeometryLoader::GetRelElementAggregates() const {
  return _relElementAggregates;
}

const std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>> &
IfcGeometryLoader::GetStyledItems() const {
  return _styledItems;
}

const std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>> &
IfcGeometryLoader::GetRelMaterials() const {
  return _relMaterials;
}

const std::unordered_map<uint32_t, std::vector<std::pair<uint32_t, uint32_t>>> &
IfcGeometryLoader::GetMaterialDefinitions() const {
      ...
      while (_loader.GetTokenType() != parsing::IfcTokenType::SET_END) {
        ...
        while (_loader.GetTokenType() != parsing::IfcTokenType::SET_END) {
          ...
          if (t == parsing::IfcTokenType::INTEGER) {
            _loader.StepBack();
            segment.indexs.push_back(
                static_cast<uint32_t>(_loader.GetIntArgument()));
          }
        }
      }
