...
namespace webifc::geometry
{
  ...
  {
    printf("[GetCrossSections2D({})]",expressID);
  ...
  {
   printf("[GetCrossSections3D({})]",expressID);
    auto lineType = _loader.GetLineType(expressID);
    IfcCrossSections sections;
    double scale = 1;
    if (scaled)
  ...
  {
    printf("[GetAlignmentCurve({})]",expressID);
    auto lineType = _loader.GetLineType(expressID);
  ...
  std::optional<glm::dvec4> IfcGeometryLoader::GetColor(uint32_t expressID) const
  {
   printf("[GetColor({})]",expressID);
    ...
    default:
      printf("[GetColor()] unexpected style type {}", expressID, lineType);
      break;
    }
  ...
  IfcBound3D IfcGeometryLoader::GetBound(uint32_t expressID) const
  {
    ...
    default:
      printf("[(GetBounds)] unexpected bound type {}", expressID, lineType);
      break;
    }
  ...
  IfcCurve IfcGeometryLoader::GetLoop(uint32_t expressID) const
  {
    ...
    default:
      printf("[GetLoop()] unexpected loop type {}", expressID, lineType);
      break;
    }
  ...
  {
    printf("[GetOrientedEdge({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 3);
    std::string_view orientValue = _loader.GetStringArgument();
    bool orient = orientValue == "T";
    _loader.MoveToArgumentOffset(expressID, 2);
  ...
  {
    printf("[GetVertexPoint({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
    ...
    {
      printf("[GetVertexPoint()] unexpected vertxpoint type {}", pointRef, point);
      return {};
    }
  }
  ...
  IfcCurve IfcGeometryLoader::GetEdge(uint32_t expressID) const
  {
    ...
    default:
      printf("[GetEdge())] unexpected edgecurve type {}", expressID, lineType);
      break;
    }
    return IfcCurve();
  }
  ...
  {
    printf("[GetCartesianPoint3D({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
  ...
  {
   printf("[GetCartesianPoint2D({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
    _loader.GetTokenType();
    // because these calls cannot be reordered we have to use intermediate variables
    double x = _loader.GetDoubleArgument();
  ...
  {
    printf("[ReadIfcCartesianPointList3D({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
  ...
  {
    printf("[ReadIfcCartesianPointList2D({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
    ...
    case schema::IFCCOMPOSITECURVE:
      {
        ...
        if (selfIntersects == "T")
        {
          ...
          printf("[ComputeCurve()] Self intersecting composite curve {}", expressID);
        }
    ...
    case schema::IFCLINE:
      {
        ...
        if (dimensions== 2 && trim.exist)
        {
          ...
          {
            printf("[ComputeCurve()] Unsupported trimmingselect 2D IFCLINE {}", expressID, lineType);
          }
        }
        else if (dimensions == 3 && trim.exist)
        {
          ...
          {
            printf("[ComputeCurve()] Unsupported trimmingselect 3D IFCLINE {}", expressID, lineType);
          }
        }
      break;
      }
    ...
    case schema::IFCINDEXEDPOLYCURVE:
      {
        ...
        if (_loader.GetTokenType() != parsing::IfcTokenType::EMPTY)
        {
          ...
          {
            // TODO: this is probably bad news
            printf("[ComputeCurve()] Self intersecting ifcindexedpolycurve {}", expressID);
          }
        }
        ...
        {
          printf("[ComputeCurve()] Parsing ifcindexedpolycurve in 3D is not possible {}", expressID);
        }
...
default:
  printf("[ComputeCurve()] Unsupported curve type {}", expressID, lineType);
  break;
}
...
IfcProfile IfcGeometryLoader::GetProfile(uint32_t expressID) const
{
  ...
  {
    printf("[GetProfileByLine({})]",expressID);
    auto lineType = _loader.GetLineType(expressID);
    switch (lineType)
    {
    ...
    default:
      printf("[GetProfileByLine()] unexpected profile type {}", expressID, lineType);
      break;
    }
  ...
  IfcProfile IfcGeometryLoader::GetProfile3D(uint32_t expressID) const
  {
   printf("[GetProfile3D({})]",expressID);
    ...
    default:
      printf("[GetProfilebyLine()] unexpected 3D profile type {}", expressID, lineType);
      break;
    }
  ...
  {
    printf("[GetAxis2Placement2D({})]",expressID);
    auto lineType = _loader.GetLineType(expressID);
    switch (lineType)
    {
    ...
    default:
      printf("[GetAxis2DPlacement()] unexpected 2D placement type {}", expressID, lineType);
      break;
    }
    return glm::dmat3();
  }
  ...
  {
    printf("[GetLocalCurve({})]",expressID);
  ...
  {
    printf("[GetLocalPlacement({})]",expressID);
    auto lineType = _loader.GetLineType(expressID);
    switch (lineType)
    {
  ...
  case schema::IFCAXIS2PLACEMENT3D:
    ...
    default:
     printf("[GetLocalPlacement()] unexpected placement type {}", expressID, lineType);
      break;
    }
  ...
  {
    printf("[GetAxis1Placement({})]",expressID);
    _loader.MoveToArgumentOffset(expressID, 0);
    uint32_t locationID = _loader.GetRefArgument();
    parsing::IfcTokenType dirToken = _loader.GetTokenType();
  ...
  void IfcGeometryLoader::ReadLinearScalingFactor()
  {
    ...
    if (projects.size() != 1)
    {
      printf("[ReadLinearScalingFactor()] unexpected empty ifc project");
      return;
    }
