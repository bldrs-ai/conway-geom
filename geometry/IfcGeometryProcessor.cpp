...
IfcGeometryProcessor::IfcGeometryProcessor(
    ...
    uint16_t circleSegments, bool coordinateToOrigin, bool optimizeprofiles)
    : _geometryLoader(loader, errorHandler, schemaManager, circleSegments),
      _loader(loader),
      _errorHandler(errorHandler),
      _schemaManager(schemaManager),
      _coordinateToOrigin(coordinateToOrigin),
      _optimize_profiles(optimizeprofiles),
      _circleSegments(circleSegments) {
  IfcProfile profile;
  double scaling = 1;
  profile.curve = GetCircleCurve(scaling, _circleSegments, glm::dmat3(1));
  predefinedCylinder =
      Extrude(profile, glm::dvec3(0, 0, 1), scaling, _errorHandler);

  IfcProfile profileCube;
  profileCube.curve = GetRectangleCurve(scaling, scaling, glm::dmat3(1));
  predefinedCube =
      Extrude(profileCube, glm::dvec3(0, 0, 1), scaling, _errorHandler);
}
...
void IfcGeometryProcessor::Clear() { _expressIDToGeometry = {}; }
...
IfcComposedMesh IfcGeometryProcessor::GetMesh(uint32_t expressID) {
  ...
  if (_schemaManager.IsIfcElement(lineType)) {
      ...
      for (auto relAggExpressID : relAggIt->second) {
        ...
        if (relVoidsIt2 != relVoids.end() && !relVoidsIt2->second.empty()) {
          if (relVoidsIt != relVoids.end() && !relVoidsIt->second.empty()) {
            relVoidsIt->second.insert(relVoidsIt->second.end(),
                                      relVoidsIt2->second.begin(),
                                      relVoidsIt2->second.end());
          } else {
    ...
    if (relVoidsIt != relVoids.end() && !relVoidsIt->second.empty()) {
      ...
      auto flatElementMeshes =
          flatten(mesh, _expressIDToGeometry, normalizeMat);
      ...
      if (flatElementMeshes.size() != 0) {
        ...
        for (auto relVoidExpressID : relVoidsIt->second) {
          ...
          auto flatVoidMesh =
              flatten(voidGeom, _expressIDToGeometry, normalizeMat);
          voidGeoms.insert(voidGeoms.end(), flatVoidMesh.begin(),
                           flatVoidMesh.end());
        }
    ...
    switch (lineType) {
      ...
      case schema::IFCSECTIONEDSURFACE: {
        auto geom = SectionedSurface(
            _geometryLoader.GetCrossSections3D(expressID), _errorHandler);
        mesh.transformation = glm::dmat4(1);
      ...
      case schema::IFCBOOLEANCLIPPINGRESULT: {
        ...
        auto flatFirstMeshes =
            flatten(firstMesh, _expressIDToGeometry, normalizeMat);
        auto flatSecondMeshes =
            flatten(secondMesh, _expressIDToGeometry, normalizeMat);

        IfcGeometry resultMesh =
            BoolSubtract(flatFirstMeshes, flatSecondMeshes);
      ...
      case schema::IFCBOOLEANRESULT: {
        ...
        if (op != "DIFFERENCE") {
          _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                    "Unsupported boolean op " + std::string(op),
                                    expressID);
          return mesh;
        }
      ...
      case schema::IFCPOLYGONALBOUNDEDHALFSPACE: {
        ...
        bool flipWinding = false;
        double extrudeDistance = EXTRUSION_DISTANCE_HALFSPACE_M /
        ...
        if (ignoreDistanceInExtrude) {
          // spec says this should be * 0, but that causes issues for degenerate
          // 0 volume pbhs hopefully we can get away by just inverting it
          extrudeDistance *= -1;
          flipWinding = true;
        }
        ...
        auto geom = Extrude(profile, extrusionNormal, extrudeDistance,
                            _errorHandler, localPlaneNormal, localPlanePos);
      ...
      case schema::IFCPOLYGONALFACESET: {
        ...
        auto points =
            _geometryLoader.ReadIfcCartesianPointList3D(coordinatesRef);
        ...
        if (_loader.GetTokenType() == parsing::IfcTokenType::SET_BEGIN) {
          _errorHandler.ReportError(
              utility::LoaderErrorType::UNSUPPORTED_TYPE,
              "Unsupported IFCPOLYGONALFACESET with PnIndex", expressID);
        }
      ...
      case schema::IFCTRIANGULATEDFACESET: {
        ...
        if (_loader.GetTokenType() == parsing::IfcTokenType::SET_BEGIN) {
          ...
          // ignore
          // std::cout << "Unsupported IFCTRIANGULATEDFACESET with PnIndex!" <<
          // std::endl;
        }
      ...
      case schema::IFCSURFACECURVESWEPTAREASOLID: {
        ...
        double dst = glm::distance(
            directrix.points[0], directrix.points[directrix.points.size() - 1]);
        ...
        IfcGeometry geom =
            Sweep(_geometryLoader.GetLinearScalingFactor(), closed, profile,
                  directrix, surface.normal(), true);
      ...
      case schema::IFCSWEPTDISKSOLID: {
        ...
        // TODO: prevent self intersections in Sweep function still not working
        // properly
        ...
        if (_loader.GetTokenType() == parsing::IfcTokenType::REAL) {
          _errorHandler.ReportError(
              utility::LoaderErrorType::UNSUPPORTED_TYPE,
              "Inner radius of IFCSWEPTDISKSOLID currently not supported",
              expressID);
          _loader.StepBack();
        ...
        IfcGeometry geom = SweepCircular(
            _geometryLoader.GetLinearScalingFactor(), mesh, _optimize_profiles,
            closed, profile, radius, directrix);
      ...
      case schema::IFCREVOLVEDAREASOLID: {
        ...
        glm::dvec3 axis =
            _geometryLoader.GetAxis1Placement(axis1PlacementID)[0];
        ...
        if (!profile.isComposite) {
          geom = Sweep(_geometryLoader.GetLinearScalingFactor(), closed,
                       profile, directrix, axis);
        } else {
          for (uint32_t i = 0; i < profile.profiles.size(); i++) {
            IfcGeometry geom_t =
                Sweep(_geometryLoader.GetLinearScalingFactor(), closed,
                      profile.profiles[i], directrix, axis);
            geom.AddPart(geom_t);
      ...
      case schema::IFCEXTRUDEDAREASOLID: {
        ...
        if (_optimize_profiles) {
          // std::cout << "Optimizing profile(ID: " << profileID << ")" <<
          // std::endl;
          if (lineProfileType == schema::IFCCIRCLEHOLLOWPROFILEDEF ||
              lineProfileType == schema::IFCCIRCLEPROFILEDEF) {
            ...
            // double thickness = _loader.GetDoubleArgument(); // Read this
            // property only in hollow profiles

            if (placementID) {
              mesh.transformation =
                  _geometryLoader.GetLocalPlacement(placementID);
            }

            glm::dmat4 profileTransform =
                glm::dmat4(glm::dvec4(1, 0, 0, 0), glm::dvec4(0, 1, 0, 0),
                           glm::dvec4(0, 0, 1, 0), glm::dvec4(0, 0, 0, 1));
            if (profilePlacementID) {
              auto trans2d =
                  _geometryLoader.GetAxis2Placement2D(profilePlacementID);
              profileTransform =
                  glm::dmat4(glm::dvec4(trans2d[0][0], trans2d[0][1], 0, 0),
            ...
            glm::dmat4 profileScale = glm::dmat4(
                glm::dvec4(dx * radius, 0), glm::dvec4(dy * radius, 0),
                glm::dvec4(0, 0, 1, 0), glm::dvec4(0, 0, 0, 1));

            glm::dmat4 extrusionScale =
                glm::dmat4(glm::dvec4(1, 0, 0, 0), glm::dvec4(0, 1, 0, 0),
                           glm::dvec4(dz * depth, 0), glm::dvec4(0, 0, 0, 1));
            ...
            mesh.hasGeometry = true;
            return mesh;
          } else if (lineProfileType == schema::IFCRECTANGLEHOLLOWPROFILEDEF ||
                     lineProfileType == schema::IFCRECTANGLEPROFILEDEF) {
            _loader.MoveToArgumentOffset(profileID, 0);
            _loader.MoveToArgumentOffset(profileID, 2);
            uint32_t profilePlacementID = _loader.GetRefArgument();
            ...
            glm::dmat4 profileScale =
                glm::dmat4(glm::dvec4(dx * dimx, 0), glm::dvec4(dy * dimy, 0),
                           glm::dvec4(0, 0, 1, 0), glm::dvec4(0, 0, 0, 1));
...
// TODO: correct dump in case of compositeProfile
#ifdef CSG_DEBUG_OUTPUT
        io::DumpSVGCurve(profile.curve.points,
                         "IFCEXTRUDEDAREASOLID_curve.html");
#endif
        ...
        if (!profile.isComposite) {
          ...
          for (uint32_t i = 0; i < profile.profiles.size(); i++) {
            IfcGeometry geom_t =
                Extrude(profile.profiles[i], dir, depth, _errorHandler);
            if (flipWinding) {
...
// TODO: correct dump in case of compositeProfile
      ...
      default:
        _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                  "unexpected mesh type", expressID, lineType);
        break;
    }
  }
...
IfcSurface IfcGeometryProcessor::GetSurface(uint32_t expressID) {
  ...
  switch (lineType) {
    ...
    case schema::IFCBSPLINESURFACEWITHKNOTS: {
      ...
      if (UKnots[UKnots.size() - 1] != (int)UKnots[UKnots.size() - 1]) {
        for (uint32_t i = 0; i < UKnots.size(); i++) {
          UKnots[i] =
              UKnots[i] * (UKnots.size() - 1) / UKnots[UKnots.size() - 1];
        }
      }
      ...
      if (VKnots[VKnots.size() - 1] != (int)VKnots[VKnots.size() - 1]) {
        for (uint32_t i = 0; i < VKnots.size(); i++) {
          VKnots[i] =
              VKnots[i] * (VKnots.size() - 1) / VKnots[VKnots.size() - 1];
        }
      }
      ...
      //  {
      //      newCtrolPts.push_back(ctrolPts[ctrolPts.size() - 1 + (i -
      //      Udegree)]);
      //  }
      ...
      //      {
      //          newSubList.push_back(ctrolPts[r][ctrolPts[r].size() - 1 + (i -
      //          Vdegree)]);
      //      }
    ...
    case schema::IFCSURFACEOFREVOLUTION: {
      ...
      surface.RevolutionSurface.Direction =
          _geometryLoader.GetLocalPlacement(locationID);
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected surface type", expressID, lineType);
      break;
  }
...
IfcFlatMesh IfcGeometryProcessor::GetFlatMesh(uint32_t expressID) {
  ...
  glm::dmat4 mat =
      glm::scale(glm::dvec3(_geometryLoader.GetLinearScalingFactor()));

  AddComposedMeshToFlatMesh(flatMesh, composedMesh,
                            _transformation * NormalizeIFC * mat);

  return flatMesh;
}

void IfcGeometryProcessor::AddComposedMeshToFlatMesh(
    IfcFlatMesh &flatMesh, const IfcComposedMesh &composedMesh,
    const glm::dmat4 &parentMatrix, const glm::dvec4 &color, bool hasColor) {
  ...
  if (composedMesh.hasGeometry) {
    ...
    _expressIDToGeometry[composedMesh.expressID] =
        *static_cast<IfcGeometry *>(&normalizedGeom);
  ...
  for (auto &c : composedMesh.children) {
    AddComposedMeshToFlatMesh(flatMesh, c, newMatrix, newParentColor,
                              newHasColor);
  }
}

IfcGeometry IfcGeometryProcessor::BoolSubtract(
  ...
  for (auto &firstGeom : firstGeoms) {
    ...
    for (auto &secondGeom : secondGeoms) {
      ...
      if (secondGeom.numFaces == 0) {
        _errorHandler.ReportError(utility::LoaderErrorType::BOOL_ERROR,
                                  "bool aborted due to empty source or target");
...
void IfcGeometryProcessor::ReadIndexedPolygonalFace(
    uint32_t expressID, std::vector<IfcBound3D> &bounds,
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected indexedface type", expressID,
                                lineType);
      break;
  }
}
...
IfcGeometry IfcGeometryProcessor::GetBrep(uint32_t expressID) {
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected shell type", expressID, lineType);
      break;
  }
...
void IfcGeometryProcessor::AddFaceToGeometry(uint32_t expressID,
  ...
  switch (lineType) {
    ...
    default:
      _errorHandler.ReportError(utility::LoaderErrorType::UNSUPPORTED_TYPE,
                                "unexpected face type", expressID, lineType);
      break;
  }
}
